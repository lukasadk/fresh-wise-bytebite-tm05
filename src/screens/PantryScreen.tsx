...
import { formatQuantity, getExpiryInfo, deletePantryItem, PantryItem } from '../data/pantryItems';
import { recordOutcome, WASTE_REASON_TO_BACKEND, WasteReasonLabel } from '../data/logs';
...
export default function PantryScreen({ navigation, route }: any) {
  ...
  const { items, loading: itemsLoading, error: itemsError, refresh } = usePantryItems();
  ...
  const sortedItems = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) =>
      sortOrder === 'soonest' ? a.expiry.daysLeft - b.expiry.daysLeft : b.expiry.daysLeft - a.expiry.daysLeft
    );
  ...
  const closestAttentionLabel =
    attentionItems.length > 0
      ? [...attentionItems].sort((a, b) => a.expiry.daysLeft - b.expiry.daysLeft)[0].expiry.rowExpiryLabel.toLowerCase()
      : null;
  ...
  const handleBulkWasted = (reason: WasteReasonLabel) => {
    ...
    runBulk(async (id) => {
      ...
      await recordOutcome({
        ...
        wasteReason: WASTE_REASON_TO_BACKEND[reason],
      });
    });
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {itemsError ? (
        <View style={styles.toast}>
          <Text style={styles.toastText}>{itemsError}</Text>
        </View>
      ) : null}
      <FlatList
        ...
        ListEmptyComponent={
          itemsLoading ? null : items.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Button label="Add food" onPress={() => navigation.navigate('AddFood')} />
            </View>
          ) : (
            <View style={styles.emptyState}>
